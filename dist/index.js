...
var constructionProjects = mysqlTable("construction_projects", {
  id: int("id").autoincrement().primaryKey(),
...
var projectInputSchema = z2.object({
  ...
  client: z2.string().trim().min(1).max(255),
  location: z2.string().trim().min(1).max(255),
  projectType: z2.string().trim().min(1).max(100),
  grade: z2.string().trim().min(1).max(50),
  area: z2.number().int().positive().max(1e8),
  duration: z2.string().trim().min(1).max(100),
  description: z2.string().trim().min(1).max(5e3),
  status: z2.enum(["completed", "in_progress"])
});
var appRouter = router({
  ...
  projects: router({
    ...
    update: adminProcedure.input(z2.object({ id: z2.number().int().positive(), data: projectInputSchema })).mutation(async ({ input }) => {
